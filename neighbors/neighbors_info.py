...
def main(inputs, duration, model_file):
    ...
    x = pandasDF[['id','name','longitude','latitude','i']]
    ...
    distances, points = clt.radius_neighbors(test_data, duration, return_distance=True)
    test['distances'] = distances.tolist()
    test['points'] = points.tolist()
    ...
    res = []
    for name, lo1, la1, i, dis, points in test.values[:, 1:7]:
        ...
        for ps in points:
            ...
            for p, d in zipped:
                ...
                id, name2, lo2, la2 = x.loc[p].values[0:4]
                res.append((name, lo1, la1, id, name2, lo2, la2, d))
    res = pd.DataFrame(res, columns=['store','initial longitude', 'initial latitude', 'neighbor id', 'neighbor store', 'neighbor longitude', 'neighbor latitude', 'distance'])
...
if __name__ == "__main__":
    spark = SparkSession.builder.appName('Neighbors info').getOrCreate()
